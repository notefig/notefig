/**
 * Vendored from @fellow/prosemirror-recreate-transform 1.2.3 (Apache-2.0;
 * see LICENSE and NOTICE beside this file), the maintained-longest fork of
 * fiduswriter/prosemirror-recreate-steps. Dormant upstream since 2022, so
 * it lives in-tree. recreateTransform(fromDoc, toDoc) rebuilds a minimal
 * ProseMirror step sequence between two documents — the diff engine behind
 * differential adoption (see components/editor/adopt-external-content.ts).
 *
 * Modifications from upstream:
 *  - the package's eight modules are combined into this one file
 *    (types, copy, getFromPath, removeMarks, getReplaceStep,
 *    simplifyTransform, recreateTransform, index);
 *  - imports rewritten from prosemirror-model / prosemirror-transform to
 *    @tiptap/pm/model / @tiptap/pm/transform so the vendored code shares
 *    the app editor's ProseMirror instance;
 *  - helpers other than recreateTransform/RecreateTransform/Options are no
 *    longer exported.
 */
 ...
 */
function getFromPath(obj: JSONValue, path: string): JSONValue {
...
// ─── simplifyTransform.ts ──────────────────────────────────────

// join adjacent ReplaceSteps
function simplifyTransform(tr: Transform) {
    if (!tr.steps.length) {
        return undefined;
    }
    ...
    while (oldSteps.length) {
        ...
        while (oldSteps.length && step.merge(oldSteps[0])) {
            ...
            if (
                ...
                step = getReplaceStep(
                    newTr.doc,
                    addedStep.apply(step.apply(newTr.doc).doc as Node)
                        .doc as Node
                ) as Step;
            } else {
    ...
    addSetNodeMarkup() {
        ...
        if (start != null) {
            ...
            try {
                ...
                if (
                    nodeType &&
                    (e as Error).message.includes("Invalid content")
                ) {
