...
// "rendered in the composing phase", not its internals.
vi.mock("@/components/agent/prompt-editor", () => ({
  PromptEditor: () => createElement("div", { "data-testid": "composer" }),
}));

// Markdown renders its HTML off-thread and lands after an effect, so a
// synchronous render shows nothing. The face's contract is which text it
// hands down; the renderer has its own tests.
vi.mock("@/components/ui/markdown", () => ({
  Markdown: ({ text }: { text: string }) => createElement("div", null, text),
}));

import { PromptBlobFace } from "@/components/agent/prompt-blob";
import type { BlobPhase } from "@/components/agent/prompt-blob-state";
import type { AgentTaskRow, AgentTurn } from "@/agent/agent-collections";
