 ...
 * official ACP library's ClientSideConnection consumes. Frames pass through
 * untouched: the library validates each notification against its schema and
 * drops what fails, so a new adapter/schema drift shows up as a missing
 * update, never as a crash (MET-104 had one such repair here for 0.4.5's
 * `rawOutput: object` — the 1.x schema types it `unknown`).
 */
