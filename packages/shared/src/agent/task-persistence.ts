 ...
 * (desktop agent-collections.ts). It lives in @notefig/shared because the CLI
 * writes the same rows into the same database and must leave them in the
 * shape the app's boot mapping expects — SQLite *is* its source of truth, so there is no
 ...
 */
import { z } from "zod";
import type { AgentTaskRow } from "./task-state";

// The collection id lives with the other shared collection identities
// (@notefig/shared/persistence), because more than one process opens it.
...
export const PersistedAgentTaskSchema = z
  .object({
    ...
    // A CLI run's pid while it owns the task (AgentTaskRow.ownerPid).
...
/**
 * A task's title from the prompt that named it: the text itself, cut to 60
 * characters. The desktop strips a leading reference blockquote before
 * calling this; the rule for what a title looks like lives here once.
 */
